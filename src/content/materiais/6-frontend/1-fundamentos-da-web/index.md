...
![Diagrama mostrando a troca de mensagens entre cliente e servidor: cliente enviando uma requisição (request) e servidor respondendo com uma resposta (response) contendo status, headers e body](/api/materiais-assets/6-frontend/1-fundamentos-da-web/assets/image.png)
...
![Pipeline visual do caminho completo de uma navegação web: URL → DNS (resolução de domínio) → Conexão TCP/TLS → HTTP Request → HTTP Response → Renderização no navegador](/api/materiais-assets/6-frontend/1-fundamentos-da-web/assets/image-1.png)
...
![Mini fluxograma de status codes HTTP agrupados por categoria: 2xx (sucesso em verde), 3xx (redirecionamento em azul), 4xx (erro do cliente em laranja) e 5xx (erro do servidor em vermelho)](/api/materiais-assets/6-frontend/1-fundamentos-da-web/assets/image-2.png)
...
![Diagrama mostrando o navegador carregando um arquivo HTML que contém uma tag link referenciando um arquivo CSS externo, ilustrando a dependência e relação entre os dois arquivos](/api/materiais-assets/6-frontend/1-fundamentos-da-web/assets/image-3.png)
