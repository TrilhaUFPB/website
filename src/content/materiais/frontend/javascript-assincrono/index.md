...
## 1. Por que assincronicidade existe no JavaScript

### 1.1 Operações que demoram e a UI responsiva
...
### 1.2 Modelo mental: “iniciar agora, continuar depois”
...
## 2. Promises (fundamento)

### 2.1 O que é uma Promise
...
### 2.2 Estados: pending, fulfilled, rejected
...
### 2.3 then/catch/finally e encadeamento
...
### 2.4 Resolver vs rejeitar (e lançar erro)
...
## 3. async/await (sintaxe mais legível)
...
### 3.2 await e a “pausa” que não bloqueia a página
...
### 3.3 try/catch/finally com await
...
### 3.4 Comparação didática: then/catch vs async/await (mesma lógica)
...
## 4. fetch (rede no navegador)

### 4.1 fetch retorna Promise de Response
...
### 4.2 Response: ok, status, headers
...
### 4.5 Diferença essencial: erro de rede vs erro HTTP
...
## 5. Consumir APIs (modelo mental de cliente)

### 5.1 O que é uma API no contexto web
...
### 5.2 Request/Response com JSON (reforço)
...
### 5.3 Query string e path params (visão geral)
...
### 5.4 CORS: por que acontece e limites
...
### 5.5 Boas práticas: organização mínima sem virar framework
...
### 6.2 Estratégia mínima de erros (sem paranoia, mas correta)
...
### 6.3 finally e noções de UX: loading, empty, error
...
### 6.4 Exemplo “padrão de mercado” (função async completa, curta e realista)
...
## 7. Boas práticas e limites do mundo real (visão geral útil)
...
## 8. Erros comuns e confusões clássicas
