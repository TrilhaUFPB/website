...
import { useVoting } from '@/hooks/trilhurna/useVoting';
...
export const VotingSystem = () => {
  ...
  const calculatePercentage = (votes: number) => {
    if (votingData.totalVotes === 0) return 0;
    return Math.round((votes / votingData.totalVotes) * 100);
  };

  // Ensure options is always an array
  const options = Array.isArray(votingData.options) ? votingData.options : [];
    ...
    <div className="max-w-2xl mx-auto p-6 bg-white rounded-lg shadow-lg">
      <div className="text-center mb-8">
        ...
        <p className="text-gray-600">
          Which do you prefer? Vote and see real-time results!
        </p>
        <div className="mt-4 text-sm text-gray-500">
          Total votes: {votingData.totalVotes}
        </div>
      </div>
      ...
      <div className="space-y-4">
        {options.map((option) => {
          ...
          const isWinning = option.votes === Math.max(...options.map(opt => opt.votes)) && option.votes > 0;
